...
export const isAdminEmail = (email) => {
  ...
  // 🐛 ISSUE 1 DETAIL: Email comparison is case-sensitive
  // oladoyeheritage445@gmail.com ≠ OLADOYEHERITAGE445@gmail.com
  // Consider using email.toLowerCase() for case-insensitive comparison
...
export const createUserProfile = async (user, additionalData = {}) => {
  ...
  if (!userSnap.exists()) {
    ...
    // 🐛 ISSUE 1 ROOT CAUSE: Admin role is only assigned during user creation
    // If ADMIN_EMAIL is updated after user account creation, existing users 
    // won't get admin privileges. This affects oladoyeheritage445@gmail.com
    // if their account was created before being added to ADMIN_EMAIL.
    // SOLUTION NEEDED: Add mechanism to update existing user roles when 
...
export const isAdmin = (userProfile) => {
  // 🐛 ISSUE 1 CONTEXT: This function only checks the stored role in userProfile
  // It does NOT re-evaluate against current ADMIN_EMAIL configuration
  // If ADMIN_EMAIL is updated, existing users won't be recognized as admins
  // until their role is manually updated in the database
